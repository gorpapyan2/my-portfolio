export const am = {
  // Navigation
  'nav.about': 'Իմ մասին',
  'nav.work': 'Աշխատանքներ',
  'nav.experience': 'Փորձ',
  'nav.blog': 'Բլոգ',
  'nav.contact': 'Կապ',
  'nav.settings': 'Կարգավորումներ',
  'nav.admin': 'Ադմին',

  // Hero
  'hero.title': 'QA Ավտոմատացման Մասնագետ',
  'hero.subtitle': 'Ես օգնում եմ թիմերին ավելի արագ թողարկել ավելի քիչ սխալներով: Որոշ թեստային հավաքածուներ, CI/CD ինտեգրացիա և ռազմավարական որակի ավտոմատացում:',
  'hero.downloadCV': 'Ներբեռնել CV',

  // Hero Proof Points
  'hero.proof1': 'Անկայուն մակարդակ ↓ ~70% տվյալների մեկուսացման և timeout կարգավորման միջոցով',
  'hero.proof2': 'Ռեգրեսիայի ժամանակ ↓ ~40% Playwright հուսալի հավաքածուների միջոցով',
  'hero.proof3': 'CI ժամանակ ↓ զուգահեռ աշխատողների, sharding-ի և storageState-ի միջոցով',

  // Common
  'language': 'Լեզու',
  'theme': 'Թեման',
  'loading': 'Բեռնվում է...',
  'error': 'Սխալ',
  'success': 'Հաջողություն',
  'save': 'Պահպանել',
  'cancel': 'Չեղարկել',
  'delete': 'Ջնջել',
  'edit': 'Խմբագրել',
  'add': 'Ավելացնել',
  'close': 'Փակել',
  'viewAll': 'Տեսնել բոլորը',
  'proficiency': 'Տիրապետման մակարդակ',

  // Accessibility
  'accessibility.skipToContent': 'Անցնել հիմնական բովանդակությանը',
  'aria.openMenu': 'Բացել ընտրացանկը',
  'aria.closeMenu': 'Փակել ընտրացանկը',
  'aria.previousTechnology': 'Նախորդ տեխնոլոգիան',
  'aria.nextTechnology': 'Հաջորդ տեխնոլոգիան',

  // Pages
  'pages.home.title': 'Գլխավոր',
  'pages.home.featuredProjects': 'Առաջարկվող նախագծեր',
  'pages.home.latestArticles': 'Վերջին հոդվածներ',

  'pages.about.title': 'Իմ մասին',
  'pages.about.subtitle': 'QA Ավտոմատացման Մասնագետ | Որոշ թեստավորում | CI/CD ինտեգրացիա',
  'pages.about.philosophy': 'Ես կառուցում եմ հուսալի թեստավորման ավտոմատացում, որին վստահում են ինժեներները: Իմ մոտեցումը՝ մեկուսացնել տվյալները, որոշ հոսքեր անել, նվազագույն անկայունություն և օգտակար ձախողման ազդանշաններ:',

  'pages.work.title': 'Իմ աշխատանքները',
  'pages.work.subtitle': 'Թեստավորման ավտոմատացման շրջանակների և որակի ապահովման նախագծերի ցուցադրություն',

  'pages.blog.title': 'Բլոգ',
  'pages.blog.subtitle': 'Ծրագրային թեստավորման և որակի ապահովման մասին մտքեր և գաղափարներ',
  // Blog
  'blog.unavailableTitle': 'Բաժինը հասանելի չէ',
  'blog.unavailableBody': 'Այս բաժինը ներկայումս հասանելի չէ։',
  'blog.empty': 'Դեռևս բլոգի գրառումներ չկան։',
  'blog.readMore': 'Կարդալ ավելին',
  'blog.back': 'Վերադառնալ բլոգին',
  'blog.notFound.title': 'Գրառումը չի գտնվել',
  'blog.notFound.description': 'Այն բլոգի գրառումը, որը դուք փնտրում եք, գոյություն չունի կամ ջնջված է։',
  'blog.error.title': 'Չհաջողվեց բեռնել գրառումը',
  'blog.error.description': 'Այս գրառումը բեռնելիս առաջացել է սխալ։',
  'blog.error.retry': 'Կրկին փորձել',
  'blog.error.retrying': 'Կրկին փորձում...',
  'blog.noContent': 'Այս գրառման համար բովանդակություն չկա։',
  'blog.publishedOn': 'Հրապարակված',
  'blog.related.title': 'Նմանատիպ հոդվածներ',
  'blog.related.subtitle': 'Շարունակեք ուսումնասիրել ավելի շատ հոդվածներ',

  'pages.contact.title': 'Կապվեք ինձ հետ',
  'pages.contact.subtitle': 'Եկեք քննարկենք, թե ինչպես կարող եմ օգնել ձեր ծրագրային ապահովման որակն ապահովել',

  // Portfolio Navigation
  'portfolioNav.title': 'Ուսումնասիրեք իմ պորտֆոլիոն',
  'portfolioNav.subtitle': 'Նավարկեք տարբեր բաժինների միջով՝ իմ փորձ, նախագծեր և գաղափարներ իմանալու համար',
  'portfolioNav.aboutTitle': 'Իմ մասին',
  'portfolioNav.aboutDesc': 'Իմացեք իմ կրտսեությունը, փորձը և թեստավորման ավտոմատացման մասնագետ հմտությունների մասին:',
  'portfolioNav.workTitle': 'Իմ աշխատանքները',
  'portfolioNav.workDesc': 'Ուսումնասիրեք իմ թեստավորման ավտոմատացման շրջանակների, որակի ապահովման նախագծերի և տեխնիկական փորձագիտության պորտֆոլիո:',
  'portfolioNav.blogTitle': 'Բլոգ',
  'portfolioNav.blogDesc': 'Կարդացեք իմ վերջին հոդվածները և գաղափարները ծրագրային թեստավորման և որակի ապահովման պրակտիկայի մասին:',
  'portfolioNav.contactTitle': 'Կապվեք',
  'portfolioNav.contactDesc': 'Ունե՞ք հարց կամ նախագիծ կարծիքում: Կապվեք և քննարկենք, թե ինչպես կարող եմ օգնել:',

  // Statistics
  'statistics.experience': 'Փորձի Տարիներ',
  'statistics.projects': 'Նախագծեր Մատենավորված',
  'statistics.tests': 'Հիմնական Стеки',
  'statistics.expertise': 'Ազդեցության Ոլորտներ',
  'statistics.areas': 'UI և API Ավտոմատացում, CI/CD Pipeline Ինտեգրացիա, որոշ թեստային ճարտարապետություն',

  // About Me sections
  'about.professionalJourney': 'Մասնագիտական ճանապարհ',
  'about.philosophy': 'Փիլիսոփայություն',
  'about.toolbox': 'Գործիքակազմ',
  'about.fallback.journey': 'Մասնագիտական ճանապարհը շուտով կհայտնվի։',
  'about.fallback.philosophy': 'Փիլիսոփայության մանրամասները շուտով կհայտնվեն։',
  'about.fallback.toolbox': 'Գործիքակազմը շուտով կհայտնվի։',
  'about.fallback.error': 'Չհաջողվեց բեռնել բովանդակությունը։ Խնդրում ենք փորձել կրկին։',
  'about.portraitAlt': 'Գոռ Փափյանի դիմանկար',
  'about.headline': 'QA ավտոմատացման մասնագետ, որը կենտրոնանում է որոշակի թեստավորման և հուսալի մատակարարման վրա։',
  'about.keyResults.title': 'Հիմնական արդյունքներ',
  'about.keyResults.4': 'Զրոյական արտադրական թերություններ կրիտիկական հոսքերում՝ շնորհիվ հուսալի նախնական ստուգման:',
  'about.keyResults.5': 'Ավտոմատացված հիմնական օգտատիրոջ ուղիների 80%+ Playwright/TypeScript-ի միջոցով:',
  'about.keyResults.6': 'Ներդրվել է պայմանագրային թեստավորում միկրոծառայությունների համար՝ նվազեցնելով ինտեգրման խնդիրները:',
  'about.keyResults.7': 'Օպտիմալացվել են Docker-ի վրա հիմնված թեստային միջավայրերը՝ տեղային և CI հետ համահունչ լինելու համար:',
  'about.keyResults.8': 'Սահմանվել են որակի շեմեր Jenkins/GitHub Actions-ում յուրաքանչյուր PR-ի համար:',
  'about.keyResults.9': 'Միջավայրի կարգավորման ժամանակը կրճատվել է 50%-ով՝ օգտագործելով ենթակառուցվածքը որպես կոդ:',
  'about.keyResults.10': 'Բարելավվել է թեստավորման հաշվետվությունը տեսողական արտեֆակտներով և կատարողականի չափանիշներով:',
  'about.keyResults.11': 'Կրտսեր QA ինժեներների մենթորություն ժամանակակից ավտոմատացման լավագույն փորձի վերաբերյալ:',
  'about.keyResults.12': 'API թեստերի ծածկույթը բարելավվել է 60%-ով՝ ավտոմատացված փաստաթղթերի ստուգումների միջոցով:',
  'about.keyResults.empty': 'Հիմնարար արդյունքները շուտով կհայտնվեն։',
  'about.quickFacts.title': 'Կարճ տեղեկություններ',
  'about.quickFacts.location': 'Գտնվելու վայր',
  'about.quickFacts.experience': 'Փորձ',
  'about.quickFacts.focus': 'Կենտրոնացում',
  'about.quickFacts.availability': 'Հասանելիություն',
  'about.quickFacts.availabilityStatus': 'Բաց նոր հնարավորությունների համար',
  'about.languages.title': 'Լեզուներ',
  'about.experience.title': 'Փորձ',
  'about.experience.subtitle': 'Ճանապարհորդություն իմ մասնագիտական կարիերայում ծրագրային ապահովման որակի ապահովման ոլորտում',
  'about.education.title': 'Կրթություն',
  'about.education.subtitle': 'Ակադեմիական ֆոն և շարունակական ուսուցում',
  'about.skills.title': 'Հմտություններ և փորձագիտություն',
  'about.skills.subtitle': 'Ընդլայնված հմտություններ թեստավորման ավտոմատացման, CI/CD և որակի ինժեներության ոլորտում',
  'experience.title': 'Փորձ',
  'experience.keyAchievements': 'Հիմնական ձեռքբերումներ',

  // Skills
  'skills.testAutomation': 'Թեստավորման ավտոմատացում',
  'skills.manualTesting': 'Ձեռքով և հետազոտական թեստավորում',
  'skills.apiTesting': 'API թեստավորում',
  'skills.cicdDevops': 'CI/CD և DevOps',
  'skills.versionControl': 'Տարբերակների վերահսկում',
  'skills.crossBrowser': 'Խաչաձև հարթակ և վեբ',
  'skills.mobileAutomation': 'Բջջային ավտոմատացում',
  'skills.testManagement': 'Թեստավորման կառավարում և Agile',
  'skills.sqlData': 'SQL և տվյալներ',
  'skills.advanced': 'Ընդլայնված',
  'skills.intermediate': 'Միջանկյալ',
  'skills.familiar': 'Ծանոթ',

  // Contact
  'contact.getInTouch': 'Կապվեք',
  'contact.name': 'Անուն',
  'contact.email': 'Էլ. փոստ',
  'contact.message': 'Հաղորդագրություն',
  'contact.sendMessage': 'Ուղարկել հաղորդագրություն',
  'contact.sending': 'Ուղարկվում է...',
  'contact.successMessage': 'Շնորհակալություն! Ձեր հաղորդագրությունը հաջողությամբ ուղարկվեց: Ես շուտով կկապվեմ ձեզ հետ:',
  'contact.errorMessage': 'Ուփս! Ինչ-որ բան սխալ է: Խնդրում ենք փորձել ավելի ուշ:',
  'contact.validationError': 'Խնդրում ենք ստուգել ձևը սխալների համար և փորձել կրկին:',
  'contact.phone': 'Հեռախոս',
  'contact.location': 'Տեղանք',

  // Footer
  'footer.about': 'Մեր մասին',
  'footer.aboutDescription': 'QA Ավտոմատացման մասնագետ, որը կենտրոնացած է հուսալի թեստային շրջանակների և CI/CD ինտեգրացիայի վրա:',
  'footer.contact': 'Կապ',
  'footer.location': 'Երևան, Հայաստան',
  'footer.quickLinks': 'Արագ հղումներ',
  'footer.connect': 'Կապվեք',
  'footer.privacy': 'Գաղտնիության քաղաքականություն',
  'footer.copyright': '© 2024 Գոր Պապյան: Բոլոր իրավունքները պաշտպանված են:',
  'footer.builtWith': 'Ստեղծված React-ով և Vite-ով',
  'footer.terms': 'Ծառայության պայմաններ',

  // Settings
  'admin.about.markdownHint': 'Այս դաշտերում աջակցվում է Markdown (գլուխներ, ցուցակներ, ընդգծումներ, կոդ).',
  'settings.title': 'Կարգավորումներ',
  'settings.translations': 'Թարգմանությունների կառավարում',
  'settings.selectLanguage': 'Ընտրեք խմբագրման լեզուն',
  'settings.addTranslation': 'Ավելացնել թարգմանություն',
  'settings.editTranslation': 'Խմբագրել թարգմանությունը',
  'settings.deleteTranslation': 'Ջնջել թարգմանությունը',
  'settings.searchPlaceholder': 'Փնտրել թարգմանություններ...',
  'settings.filterByCategory': 'Զտել ըստ կատեգորիայի',
  'settings.allCategories': 'Բոլոր կատեգորիաները',
  'settings.importExport': 'Ներմուծում / Արտահանում',
  'settings.exportTranslations': 'Արտահանել թարգմանությունները',
  'settings.importTranslations': 'Ներմուծել թարգմանությունները',
  'settings.downloadTemplate': 'Ներբեռնել կաղապար',
  'settings.validation': 'Ստուգում',
  'settings.missingTranslations': 'Բացակայող թարգմանություններ',
  'settings.emptyValues': 'Դատարկ արժեքներ',
  'settings.key': 'Բանալի',
  'settings.value': 'Արժեք',
  'settings.deleteError': 'Չհաջողվեց ջնջել թարգմանությունը',
  'settings.saveError': 'Չհաջողվեց պահպանել թարգմանությունը',
  'settings.importError': 'Չհաջողվեց ներմուծել թարգմանությունները',

  // Technologies
  'technologies.title': 'Տեխնոլոգիաներ և Գործիքներ',
  'technologies.subtitle': 'Ժամանակակից գործիքների օգտագործում՝ ծրագրային ապահովման որակն ապահովելու համար',
  'technologies.testing': 'Թեստավորման Շրջանակներ',
  'technologies.cicd': 'CI/CD Գործիքներ',
  'technologies.versionControl': 'Տարբերակների Վերահսկում',
  'technologies.projectManagement': 'Ծրագրի Կառավարում',
  'technologies.detailsTitle': 'Մանրամասն ակնարկ',
  'technologies.impactTitle': 'Իրական ազդեցություն',

  // Loading
  'loading.welcome': 'Բարի գալուստ! Ամեն ինչ պատրաստում ենք ձեզ համար...',
  'loading.settingUp': 'Մեկ վայրկյան, կարգավորում ենք...',
  'loading.almostReady': 'Գրեթե պատրաստ է! Պատրաստում ենք ձեր փորձը...',
  'loading.patience': 'Շնորհակալություն համբերատարության համար...',
  'loading.hint': 'Խնդրում ենք սպասել, մինչ մենք բեռնում ենք ձեր նախընտրած լեզուն...',
};
